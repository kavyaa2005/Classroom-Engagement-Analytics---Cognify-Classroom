...
export function Dashboard() {
  ...
  // Build chart data for each range from weeklyTrend
  const engagementData = {
    daily: weeklyTrend.slice(-5).map(p => ({ date: p.date, engagement: p.engagement })),
    weekly: weeklyTrend.slice(-4).map((p, i) => ({ date: `Week ${i + 1}`, engagement: p.engagement })),
    monthly: weeklyTrend.slice(-3).map((p, i) => ({ date: `Month ${i + 1}`, engagement: p.engagement })),
  };

  const classrooms = data?.classrooms ?? [];
  const classData = classrooms.map((c, i) => ({
    id: c._id ?? i,
    name: c.name,
    teacher: c.teacherName ?? "—",
    students: c.studentCount ?? 0,
    engagement: c.avgEngagement ?? 0,
    status:
      (c.avgEngagement ?? 0) >= 75 ? "High" :
      (c.avgEngagement ?? 0) >= 50 ? "Medium" : "Low",
  }));

  const filteredClasses =
    filterStatus === "all"
      ? classData
      : classData.filter(c => c.status.toLowerCase() === filterStatus.toLowerCase());

  // Derive alerts from low-engagement or no-data classrooms
  const alerts: { id: string | number; message: string; time: string; urgent: boolean }[] =
    classData
      .filter(c => c.engagement < 55 && c.engagement > 0)
      .slice(0, 3)
      .map(c => ({
        id: c.id,
        message: `${c.name} engagement at ${c.engagement}% — needs attention.`,
        time: "Live",
        urgent: c.engagement < 45,
      }));

  if (alerts.length === 0 && data) {
    alerts.push({
      id: "ok",
      message: "All classrooms are performing well right now.",
      time: "Just now",
      urgent: false,
    });
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={data?.activeSessions ? `${data.activeSessions} Live` : "0 Live"}
          icon={Radio}
          color="emerald"
        ...
        <StatCard
          ...
          value={data?.overallEngagement ?? 0}
          icon={TrendingUp}
          color="amber"
          onClick={() => navigate("/reports")}
          delay={0.3}
          showProgress={true}
          progressValue={data?.overallEngagement ?? 0}
        />
      </div>
