...
interface LiveData { sessions: LiveClass[]; }

export function LiveMonitoring() {
  const { data, refetch } = useFetch<LiveData>("/api/session/live");
  const liveClasses: LiveClass[] = data?.sessions ?? [];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        >
          ...
          <p className="text-2xl font-bold text-[#14B8A6] mt-1">
            {liveClasses.length > 0 ? Math.round(liveClasses.reduce((sum, cls) => sum + cls.engagement, 0) / liveClasses.length) : 0}%
          </p>
        </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {liveClasses.length === 0 && (
          <div className="col-span-2 text-center py-16 text-[#6B7280]">
            ...
            <p className="text-lg font-medium">No live sessions currently active</p>
            <p className="text-sm mt-1">Sessions will appear here when teachers start monitoring their classes</p>
          </div>
        )}
